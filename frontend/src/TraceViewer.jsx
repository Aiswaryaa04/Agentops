...
import { fetchRunDetail, fetchFlags, analyzeRun } from "./api";
...
// Separate inner component so it can live inside ReactFlowProvider and use
// the useReactFlow() hook to manually trigger re-fitting on layout changes.
function GraphCanvas({ nodes, edges, onNodeClick, panelOpen }) {
  const { fitView } = useReactFlow();

  useEffect(() => {
    // Wait one tick for the container to actually resize (panel
    // mount/unmount) before asking React Flow to recalculate fit.
